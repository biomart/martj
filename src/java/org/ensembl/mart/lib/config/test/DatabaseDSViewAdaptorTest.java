...
public class DatabaseDSViewAdaptorTest extends Base {
  ...
  public void testDatabaseDSViewAdaptor() throws Exception {
    ...
    assertSame("DatabaseDSViewAdaptor does not return correct DatasetView for getByInternalName\n", ndsv, refdbdsva.getDatasetViewByInternalName(ndsv.getInternalName()));
    assertSame("DatabaseDSViewAdaptor does not return correct DatasetView for getByDisplayName\n", ndsv, refdbdsva.getDatasetViewByDisplayName(ndsv.getDisplayName()));
    ...
    assertTrue("DatasetView in DatabaseDatasetViewAdaptor after mod, store, and update should equal modified DatasetView loaded in from file system based on message digest\n", MessageDigest.isEqual(modDSV.getMessageDigest(), newestDSV.getMessageDigest()));
    ...
    if (refdbdsva.getDatasetViews().length > 0) {
      ...
      for (int i = 0, n = ds.length; i < n; i++) {
        DatasetView view = ds[i];
        //delete all entries for user in Mart database
