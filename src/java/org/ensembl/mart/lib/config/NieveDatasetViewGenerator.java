...
public class NieveDatasetViewGenerator {
  ...
  private static final String COMMAND_LINE_SWITCHES = "hvH:U:pP:T:D:M:d:O:R:";
  private static Logger logger = Logger.getLogger(NieveDatasetViewGenerator.class.getName());

  private static String usage() {
    return "NieveDatasetViewGenerator <OPTIONS>"
      + "\n"
  ...
  public static void main(String[] oargs) throws IOException, SQLException, ClassNotFoundException {
    ...
    try {
      ...
      if (oargs.length > 0) {
        ...
        Getopt g = new Getopt(NieveDatasetViewGenerator.class.getName(), args, COMMAND_LINE_SWITCHES);
        int c;
      ...
      if (dsName != null) {
        ...
        DatasetView dsv = DatabaseDatasetViewUtils.getNieveDatasetViewFor(dsource, dbName, dsName);
        ...
        String[] potDsvs = DatabaseDatasetViewUtils.getNieveDatasetNamesFor(dsource, dbName);
        System.out.println("Potential Datasets:");
        for (int i = 0, n = potDsvs.length; i < n; i++) {
          String potDS = potDsvs[i];
          System.out.println("\t" + potDS);
          String[] mainTables = DatabaseDatasetViewUtils.getNieveMainTablesFor(dsource, dbName, potDS);
