...
public final class TranscriptEISeqQueryRunner implements QueryRunner {
	 ...
	 * @param os
	 */
	public TranscriptEISeqQueryRunner(
		Query query,
		FormatSpec format,
		Connection conn,
		OutputStream os) {
		...
		try {
			...
			while (moreRows) {
				...
				if (limit > 0) {
					sql += " limit " + limit;
					moreRows = false;
					// assume client supplied limit is sane, and run entire query
				} else {
				...
				while (rs.next()) {
					...
					if (start > 0) {
						...
						if (!(tranatts.containsKey(Location)))
							...
							SequenceLocation tranloc =
								(SequenceLocation) tranatts.get(Location);
	...
	private final SeqWriter tabulatedWriter = new SeqWriter() {
		void writeSequences(Integer geneID) throws SequenceException {
			try {
				...
				for (Iterator tranIDiter = traniDs.keySet().iterator();
					tranIDiter.hasNext();
					) {
					Hashtable tranatts =
  ...
  private final int maxColumnLen = 80;
	...
	private Logger logger =
		Logger.getLogger(CodingSeqQueryRunner.class.getName());
