...
public class QueryTreeView extends JTree implements QueryListener {
		...
		if (query.getDatasetConfig() != null) {
			...
			if (fd != null) {
				if (fd.getDisplayName() != null) {
					fieldName = fd.getDisplayName();
				} else {
					String tmp =
						deriveDisplayNameFromOption(fd, fieldName, filter.getValue());
					if (tmp != null)
						fieldName = tmp;
	...
	/**
	 * Searches fd for a descendant Option matching the field and value and then
	 * tries to find it's dynamically assigned "ancestor" by taking into account that it
	 * might have been defined in a PushAction. 
	 * 
	 * @param fd ancestor of potential option
	 * @param fieldName target option must have option.field==field
	 * @param value target option must have option.value=value
	 * @return displayName if found, otherwise null.
	 */
	private String deriveDisplayNameFromOption(
		FilterDescription fd,
		String field,
		String value) {

		Option o = fd.getOptionByFieldNameValue(field, value);

		String displayName = null;
		if (o != null && o.getDisplayName() != null) {

			QueryFilterSettings p = o.getParent();
