...
public class MartExplorerTool {
	...
	private static Logger logger =
		Logger.getLogger(MartExplorerTool.class.getName());
	...
	private static String COMMAND_LINE_SWITCHES =
		"l:H:P:U:p:d:a:f:O:F:i:I:S:t:hvc:M:R:";
	...
	public static String usage() {
		return "MartExplorerTool <OPTIONS>"
			+ "\n"
			+ "\n-h                             - this screen"
			...
			+ "\n-c DATASET                       - dataset of query"
			+ "\n-a ATTRIBUTE                   - one or more attributes (<tableConstraint.>fieldName)"
			+ "\n-f FILTER                      - zero or more filters (<tableConstraint.>fieldName(<>=)Value)"
	...
	public static void main(String[] args) {
		...
		if (help || argnum == 0) {
			...
			validationError("Dataset must be set\n" + usage());
			...
			if (formatspec.getFormat() == FormatSpec.TABULATED
				&& formatspec.getSeparator() == null) {
				logger.info(
					"No separator specified for tabulated output, defaulting to tab separated");
	...
	public static void run() {
		...
		try {
		  ...
		  // mart configuration system
		  MartConfiguration martconf = e.getMartConfiguration();
		  Dataset ds;
		  if (martconf.containsDataset(dataset))		
	      ds = martconf.getDatasetByName(dataset);
	    else {
	    	 System.err.println("Dataset "+dataset+ " not a valid dataset\n");
	    	 return;
	    }
		  ...
		  if (seqDescription != null)
			  q.setSequenceDescription(seqDescription);
	 ...
	 * 
	 * @param attribute -- String attribute name.  May be of form fieldName, or tableConstraint.fieldName
	 * @see Query
	 * @see Attribute
	 */
	public static void addAttribute(String attribute) {
		StringTokenizer tokens = new StringTokenizer(attribute, ".", false);
		if (tokens.countTokens() == 1)
		  attributes.add(new FieldAttribute(attribute)); // fieldName
		else {
			String tableConstraint = tokens.nextToken();
			String fieldName = tokens.nextToken();
		  attributes.add(new FieldAttribute(fieldName, tableConstraint));
		}
	}
	 ...
	 *  a filter type (either tableConstraint.type or type), the condition to include in 
	 *  the where clause (=<>), and the filter value (example "chr_name=3" 
	 *  for chromosome equal to three).
	...
	public static void addFilter(String filterCondition) {
		...
		if (n != 3)
			validationError("Currently unsupported filter: " + filterCondition + usage());
		else {
			String type = tokens.nextToken();
			String qualifier = tokens.nextToken();
			String condition = tokens.nextToken();
			
			StringTokenizer typesplit = new StringTokenizer(type, ".", false);
			if (typesplit.countTokens() == 1)
			  filters.add(new BasicFilter(type, qualifier,condition));
		  else {
		  	String tableConstraint = typesplit.nextToken();
		  	String fieldName = typesplit.nextToken();
		    filters.add(new BasicFilter(fieldName, tableConstraint, qualifier, condition));		  	
		  }
		}
	}
	...
	public static void addSequenceDescription(String seqrequest) {
			...
			case 3 :
				...
				if (SequenceDescription.SEQS.contains(tmpl)) {
					...
					throw new RuntimeException(
						"Couldnt parse sequence request " + seqrequest);
				}
				break;
			case 1 :
				// type
		...
		try {
			...
			throw new RuntimeException(
				"Couldnt add Sequence Description " + e.getMessage());
		}
	}
}
