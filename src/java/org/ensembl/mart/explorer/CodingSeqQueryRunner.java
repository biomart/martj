...
public final class CodingSeqQueryRunner implements QueryRunner {
	 ...
	 * 
	 */	
	public CodingSeqQueryRunner(Query query, FormatSpec format, Connection conn, OutputStream os) {
		this.query = query;
		this.format = format;
		this.conn = conn;
	...
	public void execute(int limit) throws SequenceException, InvalidQueryException {
		...
		boolean moreRows = true; // batching output system stops when this is false
    ...
    try {
		...
		while (moreRows) {
		    ...
		    if (limit > 0) {
			    sql += " limit "+limit;
			    moreRows = false; // for client determined limits, assume they are sane, and run the entire query
		    }
		    else {
		    ...
		    logger.info( "SQL : " +sql );
			  ...
			  Integer lastTran = new Integer(0); // will hold previous tranID when new tranID is encountered
			  while ( rs.next() ) {
			  	rows++;
