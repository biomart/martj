...
public class PostgreSQLDialect extends DatabaseDialect {
	 ...
	 * as select a.*, array_to_string(array(select "); for (final Iterator i =
	 * action.getConcatTableConcatColumns().iterator(); i .hasNext();) { final
	 * Column col = (Column) i.next(); sb.append("b.");
	 * sb.append(col.getName()); if (i.hasNext()) { sb.append("||'");
	 * sb.append(columnSep); sb.append("'||"); } } sb.append(" from ");
	 * sb.append(trgtSchemaName + "." + trgtTableName + ""); sb.append(" as b
	 * where "); for (int i = 0; i < action.getConcatTableFKColumns().size();
	 * i++) { if (i > 0) sb.append(" and "); final String pkColName = ((Column)
	 ...
	 * final List statements) throws Exception { final String srcSchemaName =
	 * action.getSourceTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getSourceTableSchema()) .getDatabaseSchema(); final
	 * String srcTableName = action.getSourceTableName(); final String
	 * trgtSchemaName = action.getTargetTableSchema() == null ? action
	 * .getDataSetSchemaName() : ((JDBCSchema) action.getTargetTableSchema())
	 * .getDatabaseSchema(); final String trgtTableName =
	 * action.getTargetTableName(); final boolean useGroupBy =
	 * action.getUseGroupBy(); final Collection selectCols =
	 * action.getSourceTableColumns(); final Collection expressCols =
	 * action.getTargetExpressionColumns();
	 * 
	 * statements.add("set search_path=" + action.getDataSetSchemaName() + "," +
	 * srcSchemaName + "," + trgtSchemaName + ",pg_catalog");
	 * 
	 * final StringBuffer sb = new StringBuffer(); sb.append("create table " +
	 * trgtSchemaName + "." + trgtTableName + " as select "); for (final
	 * Iterator i = selectCols.iterator(); i.hasNext();) { final Column col =
	 * (Column) i.next(); sb.append(col.getName()); sb.append(','); } // FIXME:
	 * Reinstate. /* for (final Iterator i = expressCols.iterator();
	 * i.hasNext();) { final ExpressionColumn col = (ExpressionColumn) i.next();
	 * sb.append(col.getSubstitutedExpression()); sb.append(" as ");
	 * sb.append(col.getName()); if (i.hasNext()) sb.append(','); } sb.append("
	 * from " + srcSchemaName + "." + srcTableName + ""); if (useGroupBy) {
	 * sb.append(" group by "); for (final Iterator i = selectCols.iterator();
	 * i.hasNext();) { final Column col = (Column) i.next();
	 * sb.append(col.getName()); if (i.hasNext()) sb.append(','); } }
	 * statements.add(sb.toString()); }
	 * 
	 * public void doOptimiseAddColumn(final OptimiseAddColumn action, final
	 * List statements) { final String schemaName =
	 * action.getTargetTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getTargetTableSchema()) .getDatabaseSchema(); final
	 * String tableName = action.getTargetTableName(); final String colName =
	 ...
	 * column " + colName + " integer default 0"); }
	 * 
	 * public void doOptimiseUpdateColumn(final OptimiseUpdateColumn action,
	 * final List statements) throws Exception { final String pkSchemaName =
	 * action.getTargetTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getTargetTableSchema()) .getDatabaseSchema(); final
	 * String pkTableName = action.getTargetTableName(); final String
	 * fkSchemaName = action.getCountTableSchema() == null ? action
	 * .getDataSetSchemaName() : ((JDBCSchema) action.getCountTableSchema())
	 * .getDatabaseSchema(); final String fkTableName =
	 * action.getCountTableName(); final String colName =
	 * action.getOptimiseColumnName();
	 * 
	 * statements.add("set search_path=" + action.getDataSetSchemaName() + "," +
	 * pkSchemaName + "," + fkSchemaName + ",pg_catalog");
	 * 
	 * final String countStmt = action.getUseBoolInstead() ? "case count(1) when
	 * 0 then 0 else 1 end" : "count(1)";
	 * 
	 * final StringBuffer sb = new StringBuffer(); sb.append("update " +
	 * pkSchemaName + "." + pkTableName + " set " + colName + "=(select " +
	 * countStmt + " from " + fkSchemaName + "." + fkTableName + " b where ");
	 * for (int i = 0; i < action.getTargetTablePKColumns().size(); i++) { if (i >
	 * 0) sb.append(" and "); final Column pkCol = (Column)
	 * action.getTargetTablePKColumns().get( i); final Column fkCol = (Column)
	 * action.getCountTableFKColumns() .get(i); sb.append(pkSchemaName + "." +
	 * pkTableName + "."); sb.append(pkCol.getName()); sb.append("=b.");
	 * sb.append(fkCol.getName()); } for (int i = 0; i <
	 * action.getCountTableNotNullColumns().size(); i++) { final Column col =
	 * (Column) action.getCountTableNotNullColumns() .get(i); // Skip those in
	 * FK as already checked. if (action.getCountTableFKColumns().contains(col))
	 * continue; // Check column not null. sb.append(" and b.");
	 * sb.append(col.getName()); sb.append(" is not null"); } sb.append(')');
	 * 
	 * statements.add(sb.toString()); }
	 * 
	 * public void doOptimiseCopyColumn(final OptimiseCopyColumn action, final
	 * List statements) throws Exception { final String pkSchemaName =
	 * action.getTargetTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getTargetTableSchema()) .getDatabaseSchema(); final
	 * String pkTableName = action.getTargetTableName(); final String
	 * fkSchemaName = action.getCountTableSchema() == null ? action
	 * .getDataSetSchemaName() : ((JDBCSchema) action.getCountTableSchema())
	 * .getDatabaseSchema(); final String fkTableName =
	 * action.getCountTableName(); final String interSchemaName =
	 * action.getIntermediateTableSchema() == null ? action
	 * .getDataSetSchemaName() : ((JDBCSchema)
	 * action.getIntermediateTableSchema()) .getDatabaseSchema(); final String
	 * interTableName = action.getIntermediateTableName(); final String colName =
	 * action.getOptimiseColumnName();
	 * 
	 * statements.add("set search_path=" + action.getDataSetSchemaName() + "," +
	 * pkSchemaName + "," + fkSchemaName + ",pg_catalog");
	 ...
	 * final StringBuffer sb = new StringBuffer();
	 * 
	 * if (interTableName.equals(fkTableName) && ((interSchemaName ==
	 * fkSchemaName) || (interSchemaName != null && interSchemaName
	 * .equals(fkSchemaName))) ) { // Direct link. sb.append("update " +
	 * pkSchemaName + "." + pkTableName + " set " + colName + "=(select " +
	 * countStmt + "(c." + colName + ") from " + fkSchemaName + ".`" +
	 * fkTableName + "` as c where "); for (int i = 0; i <
	 * action.getTargetTablePKColumns().size(); i++) { if (i > 0) sb.append("
	 * and "); final Column pkCol = (Column) action.getTargetTablePKColumns()
	 * .get(i); sb.append(pkSchemaName + "." + pkTableName + ".");
	 * sb.append(pkCol.getName()); sb.append("=c."); sb.append(pkCol.getName()); }
	 * sb.append(')'); } else { // Intermediate table link. sb.append("update " +
	 * pkSchemaName + "." + pkTableName + " set " + colName + "=(select " +
	 * countStmt + "(c." + colName + ") from " + fkSchemaName + "." +
	 * fkTableName + " as b inner join " + interSchemaName + "." +
	 * interTableName + " as c on "); for (int i = 0; i <
	 * action.getCountTableFKColumns().size(); i++) { if (i > 0) sb.append(" and
	 * "); final Column fkCol = (Column) action.getCountTableFKColumns()
	 * .get(i); sb.append("b."); sb.append(fkCol.getName()); sb.append("=c.");
	 * sb.append(fkCol.getName()); } sb.append(" where "); for (int i = 0; i <
	 * action.getTargetTablePKColumns().size(); i++) { if (i > 0) sb.append("
	 * and "); final Column pkCol = (Column) action.getTargetTablePKColumns()
	 * .get(i); sb.append(pkSchemaName + "." + pkTableName + ".");
	 * sb.append(pkCol.getName()); sb.append("=b."); sb.append(pkCol.getName()); }
	 * sb.append(')'); }
	 * 
	 * statements.add(sb.toString()); }
	 * 
	 * public void doPartition(final Partition action, final List statements) {
	 * final String partTableSchema = action.getTargetTableSchema() == null ?
	 * action .getDataSetSchemaName() : ((JDBCSchema)
	 * action.getTargetTableSchema()) .getDatabaseSchema(); final String
	 * partTableName = action.getTargetTableName(); final String fromTableSchema =
	 * action.getSourceTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getSourceTableSchema()) .getDatabaseSchema(); final
	 * String fromTableName = action.getSourceTableName(); final String
	 * partColumnName = action.getPartitionColumnName(); final Object
	 * partColumnValue = action.getPartitionColumnValue(); StringBuffer sb = new
	 * StringBuffer();
	 * 
	 * String escapedValue = " is null"; if (partColumnValue != null) {
	 * escapedValue = partColumnValue.toString(); escapedValue =
	 * escapedValue.replaceAll("\\\\", "\\\\"); escapedValue =
	 * escapedValue.replaceAll("'", "\\'"); escapedValue = "='" + escapedValue +
	 * "'"; } sb.append("create table " + partTableSchema + "." + partTableName + "
	 * as select ");
	 * 
	 * if (action.getSourceTablePKColumns().isEmpty()) { // Do the partition as
	 * a simple select where. for (Iterator i =
	 * action.getSourceTableAllColumns().iterator(); i .hasNext();) { final
	 * String colName = ((Column) i.next()).getName(); sb.append(colName); if
	 * (i.hasNext()) sb.append(','); } sb.append(" from " + fromTableSchema +
	 * "." + fromTableName + " where "); } else { // Do the partition as a
	 * self-leftjoin-self so that // we don't lose any foreign keys.
	 * sb.append("distinct "); final List remainingCols = new ArrayList(action
	 * .getSourceTableAllColumns());
	 * remainingCols.removeAll(action.getSourceTableFKColumns()); for (Iterator
	 * i = action.getSourceTableFKColumns().iterator(); i .hasNext();) { final
	 * String colName = ((Column) i.next()).getName(); sb.append("a." +
	 * colName); sb.append(","); } for (Iterator i = remainingCols.iterator();
	 * i.hasNext();) { final String colName = ((Column) i.next()).getName();
	 * sb.append("b." + colName + ""); if (i.hasNext()) sb.append(','); } //
	 * select a.FK cols and b.Remaining cols sb.append(" from " +
	 * fromTableSchema + "." + fromTableName + " as a left join " +
	 * fromTableSchema + "." + fromTableName + " as b on "); for (Iterator i =
	 * action.getSourceTablePKColumns().iterator(); i .hasNext();) { final
	 * String joinColName = ((Column) i.next()).getName(); sb .append("a." +
	 * joinColName + "=b." + joinColName + ""); sb.append(" and "); }
	 * sb.append("b."); }
	 * 
	 * sb.append(partColumnName); sb.append(escapedValue);
	 * statements.add(sb.toString()); }
	 * 
	 * public void doPlaceHolder(final PlaceHolder action, final List
	 * statements) { statements.add("--"); }
	 * 
	 * public void doReduce(final Reduce action, final List statements) throws
	 * Exception { final String srcSchemaName = action.getSourceTableSchema() ==
	 * null ? action .getDataSetSchemaName() : ((JDBCSchema)
	 * action.getSourceTableSchema()) .getDatabaseSchema(); final String
	 * srcTableName = action.getSourceTableName(); final String trgtSchemaName =
	 * action.getReduceTableSchema() == null ? action .getDataSetSchemaName() :
	 * ((JDBCSchema) action.getReduceTableSchema()) .getDatabaseSchema(); final
	 * String trgtTableName = action.getReduceTableName(); final String
	 * reduceSchemaName = action.getTargetTableSchema() == null ? action
	 * .getDataSetSchemaName() : ((JDBCSchema) action.getTargetTableSchema())
	 * .getDatabaseSchema(); final String reduceTableName =
	 * action.getTargetTableName();
	 * 
	 * statements.add("set search_path=" + action.getDataSetSchemaName() + "," +
	 ...
	 * 
	 * final StringBuffer sb = new StringBuffer(); sb.append("create table " +
	 * reduceSchemaName + "." + reduceTableName + " as select distinct "); final
	 * List remainingCols = new ArrayList(action .getReduceTableAllColumns());
	 * remainingCols.removeAll(action.getReduceTableFKColumns()); // Select
	 * source.PK and target.remaining for (Iterator i =
	 * action.getSourceTablePKColumns().iterator(); i .hasNext();) { final
	 * String colName = ((Column) i.next()).getName(); sb.append("a." + colName +
	 * ""); sb.append(','); } for (Iterator i = remainingCols.iterator();
	 * i.hasNext();) { final String colName = ((Column) i.next()).getName();
	 * sb.append("b." + colName + ""); if (i.hasNext()) sb.append(','); }
	 * sb.append(" from " + srcSchemaName + "." + srcTableName + " as a left
	 * join " + trgtSchemaName + "." + trgtTableName + " as b on "); for (int i =
	 * 0; i < action.getReduceTableFKColumns().size(); i++) { if (i > 0)
	 * sb.append(" and "); final String pkColName = ((Column)
	 * action.getSourceTablePKColumns() .get(i)).getName(); final String
	 * fkColName = ((Column) action.getReduceTableFKColumns()
	 * .get(i)).getName(); sb.append("a." + pkColName + "=b." + fkColName + ""); }
	 * 
	 * statements.add(sb.toString()); }
	 */
